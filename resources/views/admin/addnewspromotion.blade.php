        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">@if ($service_menu_translations['add_news_promotion']) {{$service_menu_translations['add_news_promotion']}} @else Add News & Promotion @endif </li>
            </ol>
          </div>
        </div>
          ...
          <div class="col-md-6">
            <div class="card card-secondary">
              <div class="card-header">
                <h3 class="card-title">@if ($service_menu_translations['add_news_promotion']) {{$service_menu_translations['add_news_promotion']}} @else Add News & Promotion @endif</h3>
              </div>
              <form role="form" method="post" action="{{route('savenewspromotions')}}" enctype="multipart/form-data">
                @csrf
       ...
       <div class="form-row">
            <div class="col col-sm-12">
             <label for="exampleInputEmail1">@if ($service_menu_translations['title']) {{$service_menu_translations['title']}} @else Title @endif</label> 
                    <input type="text" class="form-control" id="news_promotions_title" name="news_promotions_title" placeholder="@if ($service_menu_translations['title']) {{$service_menu_translations['title']}} @else Title @endif" required="required" >
            </div>
           ...
           <div class="form-row">
            <div class="col col-sm-12">
             <label for="exampleInputEmail1">@if ($service_menu_translations['description']) {{$service_menu_translations['description']}} @else Description @endif</label>
                    <textarea class="form-control" rows="6" id="news_promotions_description" name="news_promotions_description" placeholder="Enter Description" required=""> </textarea>
            </div>
           ...
           <div class="form-row">
            ...
            <label for="exampleInputEmail1">@if ($service_menu_translations['start_date']) {{$service_menu_translations['start_date']}} @else Start Date @endif </label>
                   <input class="form-control" type="text" placeholder="@if ($service_menu_translations['start_date']) {{$service_menu_translations['start_date']}} @else Start Date @endif" id="start_date" name="start_date">
             ...
             <label for="exampleInputEmail1">@if ($service_menu_translations['end_date']) {{$service_menu_translations['end_date']}} @else End Date @endif </label>
                  <input class="form-control" type="text" placeholder="@if ($service_menu_translations['end_date']) {{$service_menu_translations['end_date']}} @else End Date @endif" id="end_date" name="end_date">
            </div>
