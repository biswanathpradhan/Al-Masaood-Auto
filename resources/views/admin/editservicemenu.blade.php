...
$service_menu_translations = getbackendTranslations('service_menu',null,$language_id);
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item"><a href="#">@if ($service_menu_translations['heading_servicemenu']) {{$service_menu_translations['heading_servicemenu']}} @else Service Menu @endif</a></li>
              <li class="breadcrumb-item active">@if ($service_menu_translations['edit_servicemenu']) {{$service_menu_translations['edit_servicemenu']}} @else Edit Service Menu @endif </li>
            </ol>
          </div>
        </div>
          ...
          <div class="col-md-6">
            <div class="card card-secondary">
              <div class="card-header">
                <h3 class="card-title">@if ($service_menu_translations['heading_servicemenu']) {{$service_menu_translations['heading_servicemenu']}} @else Service Menu @endif</h3>
              </div>
              <form role="form" method="post" action="{{route('updateServiceMenu')}}" enctype="multipart/form-data">
                @csrf
           ...
           <div class="form-row">
            <div class="col">
             <label for="exampleInputEmail1">@if ($service_menu_translations['title']) {{$service_menu_translations['title']}} @else Title @endif</label>
                    <input type="text" class="form-control" id="service_menu_title" name="service_menu_title" placeholder="@if ($service_menu_translations['enter_title']) {{$service_menu_translations['enter_title']}} @else Title @endif" required="required" @if($compact_model_val->service_menu_title) value="{{ $compact_model_val->service_menu_title }}"@endif>
            </div>
           ...
           <div class="form-row">
            <div class="col">
             <label for="exampleInputEmail1">@if ($service_menu_translations['description']) {{$service_menu_translations['description']}} @else Description @endif</label>
                    <textarea class="form-control" rows="6" id="service_menu_description" name="service_menu_description" placeholder="Enter Description" required="">@if($compact_model_val->service_menu_description) {{ $compact_model_val->service_menu_description }} @endif</textarea>
            </div>
