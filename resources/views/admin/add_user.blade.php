...
// breadcrumb_admin_manage
 ?> 
  ...
  <div class="dropdown">
        <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-cog"></span> @if ($manage_users_translations['roles']) {{$manage_users_translations['roles']}} @else Roles @endif  <span class="caret"></span></button>
  <ul class="dropdown-content checkbox-menu">
      <li>  <label style="color:#007bff"><input type="checkbox" id="select-all"> Select All</label></li>

<?php $i=0; while ( count($roles) > $i) {

 ?>
  <li> <label><input  class="form-check-label" name="role_id[]" value="<?php  echo  $roles[$i]['role_id']; ?>" type="checkbox"/>&nbsp;<?php  echo  $roles[$i]['menu_name']; ?></label></li>
