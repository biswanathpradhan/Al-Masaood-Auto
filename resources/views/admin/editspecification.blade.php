        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">@if ($specifications_translations['edit_specification']) {{$specifications_translations['edit_specification']}} @else Edit Specification @endif </li>
            </ol>
          </div>
        </div>
<!-- <div class="col-sm-6">
          ...
          <div class="col-md-6">
            <div class="card card-secondary">
              <div class="card-header">
                <h3 class="card-title">@if ($specifications_translations['edit_specification']) {{$specifications_translations['edit_specification']}} @else Edit Specification @endif</h3>
              </div>
              <form role="form" method="post" action="{{route('updatespecification')}}" enctype="multipart/form-data">
                @csrf
                  <input type="hidden" name="version_id" id="version_id" value="{{url_encode($specification->version_id)}}">
                  <input type="hidden" name="specification_id" id="specification_id" value="{{$specification_id}}">
                <input type="hidden" name="version_id_del" id="version_id_del" value="{{$specification->version_id}}">
                 
                  <input class="form-control" type="hidden" name="main_model_id" id="main_model_id" value={{url_encode($specification->version_id)}}>
              ...
              <option value="{{ $item->id }}" @if($main_brand_id == $item->id)  selected="selected" @endif> {{ strtoupper($item->main_brand_name) }} </option>
              @endforeach
                        </select>
                      </div>
              ...
              <option value="{{ $item->id }}" @if($model_id == $item->id)  selected="selected" @endif> {{ strtoupper($item->model_name) }} </option>
              @endforeach
                        </select>
                      </div>
              ...
              <option value="{{ $item->id }}" @if($version_id == $item->id)  selected="selected" @endif> {{ strtoupper($item->version_name) }} </option>
              @endforeach
                        </select>
                      </div>
              ...
              <option value="{{ $item->id }}" @if($specification->specification_category_id == $item->id)  selected="selected" @endif> {{ strtoupper($item->category_name) }} </option>
              @endforeach
                        </select>
                      </div>
                  <div class="form-group">
                    ...
                    <textarea   rows="4" cols="50" class="form-control" id="specification" name="specification" placeholder="@if ($specifications_translations['enter_specifications']) {{$specifications_translations['enter_specifications']}} @else Enter Specification @endif " required="required"> @if($specification->specification) {{ $specification->specification }} @endif</textarea>
                  </div>
                   <div class="form-group">
                    <label for="exampleInputEmail1">┘à┘ê╪º╪╡┘ü╪⌐</label>
                    <textarea   rows="4" cols="50" class="form-control" id="specification_ar" name="specification_ar" placeholder="@if ($specifications_translations['enter_specifications']) {{$specifications_translations['enter_specifications']}} @else Enter Specification @endif" required="required" > @if($specification->specification_ar) {{ $specification->specification_ar }} @endif</textarea>
                  </div> 
