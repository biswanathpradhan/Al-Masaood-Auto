...
use DB;
class emergencycallservice extends Model
{
     protected $table = 'emergency_call_service';
     protected $primaryKey = 'id';

     public static function savecar_emergencycall_request($request)
     {
        $emergencycallservice = new emergencycallservice();
        $emergencycallservice->main_brand_id = $request->main_brand_id;
        $emergencycallservice->fk_user_id = $request->customer_id;
        $emergencycallservice->date = $request->date;
        $emergencycallservice->time = $request->time;
        $emergencycallservice->latitude = $request->latitude;
        $emergencycallservice->longitude = $request->longitude;
        $emergencycallservice->save();
 ...
 //     }
    ...
    public static function get_call_back_requestcheckApi($customer_id,$session_id,$news_id)
     ...
     {
         $call_back_request = emergencycallservice::where('emergency_call_service.soft_delete', 0)->where('emergency_call_service.countstatus', 0)->count();
         return  $call_back_request;
     }

       public static function updatecountstatus()
     {
         $call_back_request = emergencycallservice::where('emergency_call_service.countstatus', 0)->update(['countstatus' => 1]);
         return  $call_back_request;
     }
}
